...
describe('AuthController (e2e)', () => {
	...
	it('/auth/register (POST)', async () => {
		const response = await request(app.getHttpServer()).post('/auth/register').send({ username, password });
	...
	it('/auth/change-password (POST) - change password', async () => {
		...
		const thirdLoginResponse = await request(app.getHttpServer()).post('/auth/login').send({ username, password });
